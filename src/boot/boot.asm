; src/boot/boot.asm
; 16bitリアルモード → A20有効化 → GDT設定 → CR0.PE=1 → 32bit保護モード
; カーネルを物理 0x00010000 にロードし、32bit C++ kernel_main へ制御を渡す
...
start:
    ...
    ;--------------------------------------
    ; カーネルを 0x00010000 に読み込む
    ;--------------------------------------
    ; ES:BX = 0x1000:0x0000 → 物理 0x1000 * 16 + 0 = 0x00010000
    mov ax, 0x1000
    mov es, ax
    xor bx, bx
...
protected_entry:
    ...
    ; カーネルエントリへジャンプ
    ; カーネルは物理アドレス 0x00010000 にリンクされている
    mov eax, 0x00010000
    call eax
...
; GDT 定義
;--------------------------------------
; フラットモデル：ベース=0, リミット≒4GB, コード/データ分離
